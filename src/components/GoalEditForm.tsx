...
import { deleteGoalAction, editGoalAction, type ActionState } from "@/app/actions";
import type { DbGoal } from "@/lib/supabase/queries";

const initialState: ActionState = {};

export function GoalEditForm({ goal }: { goal: DbGoal }) {
  const [state, formAction, pending] = useActionState(editGoalAction, initialState);
  const [name, setName] = useState(goal.name);
  const [target, setTarget] = useState(String(goal.target_minor / 100));
      ...
      >
        ...
        <Section title="Сколько нужно">
          ...
          <p className="mt-2 text-xs text-text-muted">
            Уже накоплено: {(goal.saved_minor / 100).toLocaleString("ru-RU")}{" "}
            {goal.currency === "KGS" ? "с" : "₸"}
          </p>
        </Section>
        ...
        <Section title="Опасные действия">
          <form action={deleteGoalAction} className="flex">
            <input type="hidden" name="id" value={goal.id} />
            <button
              type="submit"
